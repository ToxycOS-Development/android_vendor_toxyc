...
func init() {
	android.RegisterModuleType("toxyc_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = toxycExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = toxycExpandVariables(ctx, dep_file)
	...
	cmd := toxycExpandVariables(ctx, String(g.properties.Cmd))
